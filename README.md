...
| 版本号、在线用户、Undo/Redo 正常                                                | 在线用户、版本计数、深色模式正常                                                       | 远程用户标签、服务端持久化、离线编辑均正常                                                |
...
### Bug 7：并发编辑时光标被"劈"在中间（v3 字符级 CRDT 阶段发现）
...
| **7 并发光标劈叉** | **前端光标锚点策略（绝对位置）** | **⚠️ v3 才有能力修复（Y.Text + RelativePosition）** | **✅ 已修（Y.RelativePosition 锚点）** |
...
**A: Hello→Hello World，B: Hello→Hello AI，最终内容是什么？**  字符级 CRDT 下，**两组内容都保留**（`Hello WorldAI` 或 `Hello AIWorld`，取决于两端 clientID 排序），且**两端最终完全一致**（收敛性）。CRDT 不承诺"穿插（4x4x4x）"——因为同一个用户打的字是有先后依赖的（第 2 个 4 接在第 1 个 4 后面），所以会各自成组。CRDT 承诺的是：不丢数据 + 两端收敛一致。

## 继续开发方向

按优先级：

1. **版本快照 / 历史回退**：Y.Snapshot 实现版本记录与回退
2. **富文本格式**：Y.Xml 支持加粗 / 斜体 / 链接等内联格式
3. **评论 / 批注**：基于 Y.Array 的评论线程
4. **多文档管理**：多房间 / 文档列表
5. **精确光标渲染**：在文本中显示远程用户的光标竖线（目前只显示在哪块编辑）
