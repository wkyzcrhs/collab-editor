...
![AI 助手](docs/images/v3-ai-assistant.png)

基于 DeepSeek API 的流式 AI 助手，内置「总结文档 / 检查待办 / 建议改进 / 提取大纲」4 个快捷指令，SSE 逐字输出。当前为全量文档上下文注入，RAG 检索增强见优化方向。
...
| AI 助手            | ✅         | DeepSeek 流式回复，基于文档上下文（RAG 检索见优化方向） |
...
- 多文档管理：侧边栏 4 个文档入口目前共用一个房间，独立房间 / 文档列表未实现
- 精确光标渲染：远程用户只显示"在哪块编辑"，文本内的光标竖线未做
- AI 助手目前是全量文档上下文注入（prompt stuffing），真正的 RAG（分块 / 向量化 / 相似度检索）未实现
